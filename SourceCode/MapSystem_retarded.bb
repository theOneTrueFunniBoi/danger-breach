...
Function LoadRMesh(file$,rt.RoomTemplates)
	...
	For i=0 To 3 ;reattempt up to 3 times
		...
		EndIf
	Next
	If f=0 Then RuntimeError "Failed to read RoomMesh: "+Chr(34)+file+Chr(34)
	...
	ElseIf isRMesh$="RoomMesh.HasTriggerBox"
		hasTriggerBox% = True
	Else
		RuntimeError Chr(34)+file+Chr(34)+" is not a valid RoomMesh ("+isRMesh+")"
	EndIf
...
Function LoadSirenMesh(file$) ;stripped down rmesh
	
	LoadingWhatAsset = file$
	
	UpdateLoading() ;think of better way later
	
	CatchErrors("Uncaught (LoadSirenMesh)")
	;generate a texture made of white
	Local blankTexture%
	blankTexture=CreateTexture(4,4,1,1)
	ClsColor 255,255,255
	...
	For i=0 To 3 ;reattempt up to 3 times
		...
		EndIf
	Next
	If f=0 Then RuntimeError "Failed to read SirenMesh: "+Chr(34)+file+Chr(34)
	Local isRMesh$ = ReadString(f)
	If isRMesh$="SirenMesh"
		;Continue
	Else
		RuntimeError Chr(34)+file+Chr(34)+" is not a valid SirenMesh ("+isRMesh+")"
	EndIf
...
Type Forest
	...
	Field grid%[100+11] ;gridsize*gridsize
	Field TileEntities%[100+1]
	Field Forest_Pivot%
...
Type RoomTemplates
	...
	Field TempSoundEmitter%[8] ;maxroomemmiters
	Field TempSoundEmitterX#[8],TempSoundEmitterY#[8],TempSoundEmitterZ#[8] ;maxroomemmiters
	Field TempSoundEmitterRange#[8] ;maxroomemmiters
...
Const ZONEAMOUNT = 3
Global MapWidth% = GetINIInt(OptionFile, "options", "map width"), MapHeight% = GetINIInt(OptionFile, "options", "map height")
...
Type Rooms
	...
	Field SoundEmitter%[8] ;maxroomemitters
	Field SoundEmitterObj%[8]
	Field SoundEmitterRange#[8]
	Field SoundEmitterCHN%[8]
	
	Field Lights%[32] ;maxroomlights
	Field LightIntensity#[32] ;maxroomlights
	
	Field LightSprites%[32] ;maxroomlights	
	
	Field Objects%[30] ;maxroomobjects
	Field Levers%[11]
	...
	Field MaxLights% = 0
	Field LightSpriteHidden%[32] ;maxroomlights
	Field LightSpritesPivot%[32] ;maxroomlights
	Field LightSprites2%[32] ;maxroomlights
	Field LightHidden%[32] ;maxroomlights
	Field LightFlicker%[32] ;maxroomlights
	;Field AlarmRotor%[1] ;never used wtf
	;Field AlarmRotorLight%[1] ;never used wtf
	
	Field SpinningAlarmRotor%[30] ;maxalarmobjects
	Field SpinningAlarmLight%[30] ;maxalarmobjects
	Field SpinningAlarmStatus%[30] ;maxalarmobjects
	
	Field TriggerboxAmount
	Field Triggerbox[128]
	Field TriggerboxName$[128]
	Field MaxWayPointY#
	Field LightR#[32],LightG#[32],LightB#[32] ;maxroomlights
	Field LightCone%[32] ;maxroomlights
	Field LightConeSpark%[32] ;maxroomlights
	Field LightConeSparkTimer#[32] ;maxroomlights
...
Type Grids
	Field grid%[19] ;gridsz*gridsz
	Field angles%[19] ;gridsz*gridsz
	Field Meshes%[7]
	Field Entities%[19] ;gridsz*gridsz
	Field waypoints.WayPoints[19] ;gridsz*gridsz
End Type
...
;			CameraFogMode dp\cam,1
			...
			d = CreateDoor(r\zone, r\x + 928.0 * RoomScale,0,r\z + 640.0 * RoomScale,0,r,False,False,True)
...
;			EntityParent dp\cam,fr\Forest_Pivot
		...
		Case "lockroom"
			...
			If Rand(0,4)=1 Then sc\FurryEffect=True
		...
		Case "gatea"
			;[Block]
			r\RoomDoors[2] = CreateDoor(r\zone, r\x - 4064.0 * RoomScale, r\y-1280.0*RoomScale, r\z + 3952.0 * RoomScale, 0, r, False, False, False, "", False)
			r\RoomDoors[2]\AutoClose = False : r\RoomDoors[2]\open = False
			
			r\RoomDoors[3] = CreateDoor(r\zone, r\x + 0.0 * RoomScale, r\y+ 0.0*RoomScale, r\z + 2336.0 * RoomScale, 0, r, False, 1, False, "", False)
			r\RoomDoors[3]\AutoClose = False : r\RoomDoors[3]\open = False : r\RoomDoors[3]\locked = True
			
			FreeEntity r\RoomDoors[3]\buttons[0] : r\RoomDoors[3]\buttons[0] = 0
			
			FreeEntity r\RoomDoors[3]\buttons[1] : r\RoomDoors[3]\buttons[1] = 0
			
			d2 = CreateDoor(r\zone, r\x, r\y, r\z - 1024.0 * RoomScale, 0, r, False, False, False, "", False)
			...
			If SelectedEnding = "A2" Then 
				...
				r\RoomDoors[3]\open = True : r\RoomDoors[3]\locked = True	
			Else
				d2\AutoClose = False : d2\open = False : d2\locked = False	
				r\RoomDoors[3]\open = False : r\RoomDoors[3]\locked = True
			EndIf	
			...
			For r2.Rooms = Each Rooms
				...
				ElseIf r2\RoomTemplate\Name = "gateaentrance"
					;ylempi hissi
					r\RoomDoors[1] = CreateDoor(0, r\x+1544.0*RoomScale, r\y, r\z-64.0*RoomScale, 90, r, False, 3, False, "", False, True)
		...
		Case "gateaentrance"
			;[Block]
			;alempi hissi
			r\RoomDoors[0] = CreateDoor(0, r\x+744.0*RoomScale, 0, r\z+512.0*RoomScale, 90, r, True, 3, False, "", False, True)
		...
		Case "exit1"
			...
			;alempi hissi
			r\RoomDoors[0] = CreateDoor(0, r\x+720.0*RoomScale, 0, r\z+1432.0*RoomScale, 0, r, True, 3, False, "", False, True)
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = True
			...
			;ylempi hissi
			r\RoomDoors[1] = CreateDoor(0, r\x-5424.0*RoomScale, 10784.0*RoomScale, r\z-1380.0*RoomScale, 0, r, False, 3, False, "", False, True)
			r\RoomDoors[1]\AutoClose = False : r\RoomDoors[1]\open = False
		...
		Case "room079"
			...
			r\RoomDoors[1] = CreateDoor(0, r\x+456.0*RoomScale, -448.0*RoomScale, r\z+1616.0*RoomScale, 90, r, True, 3, False, "", False, True)
			
			r\Objects[3] = CreatePivot()
			PositionEntity(r\Objects[3], r\x + 144.0 * RoomScale, -448.0 * RoomScale, r\z+1616.0*RoomScale)
			;TurnEntity(r\Objects[3], 0,-90,0,True)
			EntityParent(r\Objects[3], r\obj)
			
			r\RoomDoors[2] = CreateDoor(0, r\x+0*RoomScale, 0, r\z+-312.0*RoomScale, 0, r, False, 3, False, "", False, True)
			
			r\Objects[4] = CreatePivot()
			PositionEntity(r\Objects[4], r\x + 0 * RoomScale, 0, r\z+0*RoomScale)
			;TurnEntity(r\Objects[4], 0,90,0,True)
			EntityParent(r\Objects[4], r\obj)
			
			r\Objects[5] = LoadAnimMesh_Strict("GFX\map\elevatordoor.b3d")
			
			ScaleEntity(r\Objects[5], RoomScale, RoomScale, RoomScale)
			PositionEntity (r\Objects[5], r\x + 152.0*RoomScale, -448.0*RoomScale, r\z+1328.0*RoomScale, True)
			EntityParent(r\Objects[5], r\obj)
			TurnEntity r\Objects[5],0,0,0,True
			
			r\Objects[6] = LoadAnimMesh_Strict("GFX\map\elevatordoor.b3d")
			
			ScaleEntity(r\Objects[6], RoomScale, RoomScale, RoomScale)
			PositionEntity (r\Objects[6], r\x + 152.0*RoomScale, -448.0*RoomScale, r\z+1328.0*RoomScale, True)
			EntityParent(r\Objects[6], r\obj)
			TurnEntity r\Objects[6],0,180,0,True
			
			r\Objects[7] = LoadAnimMesh_Strict("GFX\map\elevatordoor.b3d")
			
			ScaleEntity(r\Objects[7], RoomScale, RoomScale, RoomScale)
			PositionEntity (r\Objects[7], r\x + 288.0*RoomScale, 0.0*RoomScale, r\z+-8.0*RoomScale, True)
			EntityParent(r\Objects[7], r\obj)
			TurnEntity r\Objects[7],0,-90,0,True
			
			r\Objects[8] = LoadAnimMesh_Strict("GFX\map\elevatordoor.b3d")
			
			ScaleEntity(r\Objects[8], RoomScale, RoomScale, RoomScale)
			PositionEntity (r\Objects[8], r\x + 288.0*RoomScale, 0.0*RoomScale, r\z+-8.0*RoomScale, True)
			EntityParent(r\Objects[8], r\obj)
			TurnEntity r\Objects[8],0,90,0,True
			
			r\Objects[9] = LoadAnimMesh_Strict("GFX\map\doorframe.x")
			
			ScaleEntity(r\Objects[9], RoomScale, RoomScale, RoomScale)
			PositionEntity (r\Objects[9], r\x + 288.0*RoomScale, 0.0*RoomScale, r\z+-8.0*RoomScale, True)
			EntityParent(r\Objects[9], r\obj)
			TurnEntity r\Objects[9],0,90,0,True
			
			r\Objects[10] = LoadAnimMesh_Strict("GFX\map\doorframe.x")
			
			ScaleEntity(r\Objects[10], RoomScale, RoomScale, RoomScale)
			PositionEntity (r\Objects[10], r\x + 152.0*RoomScale, -448.0*RoomScale, r\z+1328.0*RoomScale, True)
			EntityParent(r\Objects[10], r\obj)
		...
		Case "room096"
			;[Block]
			r\RoomDoors[0] = CreateDoor(r\zone, r\x -224.0*RoomScale, 0.0*RoomScale, r\z -288.0 * RoomScale, 0, r, False, True, False)
			
			r\RoomDoors[0]\dir = 1 : r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = False
			
			PositionEntity(r\RoomDoors[0]\buttons[1], r\x + 984.0 * RoomScale, +224*RoomScale, r\z -40.0 * RoomScale, True)
			TurnEntity(r\RoomDoors[0]\buttons[0],0,-90-90,0,True)
			
			PositionEntity(r\RoomDoors[0]\buttons[0], r\x + 0.0 * RoomScale, -240*RoomScale, r\z + 0.0 * RoomScale, True)
			
			r\RoomDoors[1] = CreateDoor(r\zone, r\x -224.0*RoomScale, 0.0*RoomScale, r\z + 256.0 * RoomScale, 0, r, False, True, False)
			
			r\RoomDoors[1]\dir = 1 : r\RoomDoors[1]\AutoClose = False : r\RoomDoors[1]\open = True : r\RoomDoors[1]\locked = True
			
			PositionEntity(r\RoomDoors[1]\buttons[1], r\x + 984.0 * RoomScale, +224*RoomScale, r\z -122.0 * RoomScale, True)
			TurnEntity(r\RoomDoors[1]\buttons[0],0,-90-90,0,True)
			
			PositionEntity(r\RoomDoors[1]\buttons[0], r\x + 0.0 * RoomScale, -240*RoomScale, r\z + 0.0 * RoomScale, True)
			
			r\RoomDoors[2] = CreateDoor(r\zone, r\x + 528.0*RoomScale, 0.0*RoomScale, r\z -416.0 * RoomScale, 90, r, False, False, 4)
			
			r\RoomDoors[2]\AutoClose = False : r\RoomDoors[2]\open = True : r\RoomDoors[2]\locked = False
			
			r\RoomDoors[3] = CreateDoor(r\zone, r\x + 768.0*RoomScale, 0.0*RoomScale, r\z +240.0 * RoomScale, 0, r, False, False, 4)
			
			r\RoomDoors[3]\AutoClose = False : r\RoomDoors[3]\open = False : r\RoomDoors[3]\locked = True
			
			r\Objects[0] = CreatePivot()
			EntityParent(r\Objects[0], r\obj)
			PositionEntity (r\Objects[0], r\x -752.0*RoomScale, -448.0*RoomScale, r\z+1264.0*RoomScale, True)
			
			r\Objects[1] = CreatePivot()
			EntityParent(r\Objects[1], r\obj)
			PositionEntity (r\Objects[1], r\x -798.0*RoomScale, 0.0*RoomScale, r\z-14*RoomScale, True)
			
			it = CreateItem("Level 5 Key Card", "key5", r\x -752.0 * RoomScale, r\y + 100 * RoomScale, r\z + 1204.0 * RoomScale)
			EntityParent(it\collider, r\obj)
			;[End Block]
		Case "checkpoint1"
			;[Block]
			r\RoomDoors[0] = CreateDoor(0, r\x + 48.0*RoomScale, 0, r\z - 128.0 * RoomScale, 0, r, False, False, 3, "", False, False, True)
			...
			r\RoomDoors[1] = CreateDoor(0, r\x - 352.0*RoomScale, 0, r\z - 128.0 * RoomScale, 0, r, False, False, 3, "", False, False, True)
			;FreeEntity r\RoomDoors[1]\buttons[0]
			;FreeEntity r\RoomDoors[1]\buttons[1]
			
			r\RoomDoors[2] = CreateDoor(0, r\x - 240.0*RoomScale, 0, r\z + 720.0 * RoomScale, 90, r, False, 3, False, "", False, True)
			
			r\RoomDoors[3] = CreateDoor(0, r\x + 384.0*RoomScale, 0, r\z + 192.0 * RoomScale, 90, r, False, 3, False, "", False, True)
			
			PositionEntity(r\RoomDoors[3]\buttons[1], EntityX(r\RoomDoors[3]\buttons[1],True), EntityY(r\RoomDoors[3]\buttons[1],True), r\z + 328.0 * RoomScale, True)
			
			r\RoomDoors[1]\LinkedDoor = r\RoomDoors[0]
			r\RoomDoors[0]\LinkedDoor = r\RoomDoors[1]
			
			r\Objects[0] = CreatePivot(r\obj)
			PositionEntity (r\Objects[0], r\x -784.0*RoomScale, 120.0*RoomScale, r\z+368.0*RoomScale, True)
			;RotateEntity (r\Objects[0],0,180,0)
			...
			r\Objects[4] = CreatePivot()
			PositionEntity (r\Objects[4], r\x - 480.0*RoomScale, 0.0*RoomScale, r\z+720.0*RoomScale, True)
			
			r\Objects[5] = CreatePivot()
			PositionEntity (r\Objects[5], r\x + 624.0*RoomScale, 0.0*RoomScale, r\z+192.0*RoomScale, True)
		...
		Case "checkpoint2"
			;[Block]
			r\RoomDoors[0]= CreateDoor(0, r\x - 48.0*RoomScale, 0, r\z + 128.0 * RoomScale, 0, r, False, False, 4, "", False, False, True)
			...
			r\RoomDoors[1] = CreateDoor(0, r\x + 352.0*RoomScale, 0, r\z + 128.0 * RoomScale, 0, r, False, False, 4, "", False, False, True)
			;FreeEntity r\RoomDoors[1]\buttons[0]
			;FreeEntity r\RoomDoors[1]\buttons[1]
		...
		Case "room2testroom2"
			...
			;[End Block]
		Case "room2testpicture"
			;[Block]
			r\RoomDoors[0] = CreateDoor(r\zone, r\x + 336.0 * RoomScale, 0.0, r\z -800.0 * RoomScale, 90, r, False, False, 3)
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = False
			r\RoomDoors[0]\locked = False
			
			r\RoomDoors[1] = CreateDoor(r\zone, r\x + 336.0 * RoomScale, 0.0, r\z + 672.0 * RoomScale, 90, r, False, False, 3)
			r\RoomDoors[1]\AutoClose = False : r\RoomDoors[1]\open = False
			r\RoomDoors[1]\locked = False
			
			r\RoomDoors[2] = CreateDoor(r\zone, r\x + 0.0 * RoomScale, 0.0, r\z + 0.0 * RoomScale, 0, r, False)
			r\RoomDoors[2]\AutoClose = False : r\RoomDoors[2]\open = True
			r\RoomDoors[2]\locked = False
			
			r\RoomDoors[3] = CreateDoor(r\zone, r\x + 672.0 * RoomScale, 0.0, r\z + 0.0 * RoomScale, 0, r, False)
			r\RoomDoors[3]\AutoClose = False : r\RoomDoors[3]\open = False
			r\RoomDoors[3]\locked = False
			
			PositionEntity(r\RoomDoors[3]\buttons[0], r\x + 1257.0 * RoomScale, 231*RoomScale, r\z - 496.0 * RoomScale, True)
			TurnEntity(r\RoomDoors[3]\buttons[0],0,-90,0,True)
			PositionEntity(r\RoomDoors[3]\buttons[1], r\x + 0.0 * RoomScale, -250*RoomScale, r\z + 0.0 * RoomScale, True)
			TurnEntity(r\RoomDoors[3]\buttons[1],0,90,0,True)
			
			r\Objects[0] = CreatePivot()
			PositionEntity(r\Objects[0], r\x + 832.0 * RoomScale, 160.0 * RoomScale, r\z + 544.0 * RoomScale)
			EntityParent(r\Objects[0], r\obj)
			
			r\Objects[1] = CreatePivot()
			PositionEntity(r\Objects[1], r\x + 700.0 * RoomScale, 0.0 * RoomScale, r\z + 400.0 * RoomScale)
			EntityParent(r\Objects[1], r\obj)
			
			;it = CreateItem("Scribbled Document", "paper", r\x - 672.0 * RoomScale, r\y + 240.0 * RoomScale, r\z + 224.0 * RoomScale)
			;EntityParent(it\collider, r\obj)
			;PositionEntity(it\collider, r\x + 0.0 * RoomScale, 32*RoomScale, r\z + 768.0 * RoomScale, True)
			
			it = CreateItem("Level 1 Key Card", "key1", r\x - 672.0 * RoomScale, r\y + 240.0 * RoomScale, r\z + 224.0 * RoomScale)
			EntityParent(it\collider, r\obj)
			PositionEntity(it\collider, r\x + 1225.0 * RoomScale, 100*RoomScale, r\z - 761.0 * RoomScale, True)
			TurnEntity(it\collider,0,70,0,True)
			;[End Block]
		Case "room2testmisc"
			;[Block]
			r\RoomDoors[0] = CreateDoor(r\zone, r\x + 336.0 * RoomScale, 0.0, r\z -800.0 * RoomScale, 90, r, False, False, 3)
			...
			r\RoomDoors[3]\AutoClose = False : r\RoomDoors[3]\open = False
			;[End Block]
		Case "room2generators"
			;[Block]
			
			Local lBase%
			
			r\RoomDoors[0] = CreateDoor(r\zone, r\x + 0 * RoomScale, 0.0, r\z -512.0 * RoomScale, 0, r, False)
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = False
			
			r\RoomDoors[1] = CreateDoor(r\zone, r\x + 0 * RoomScale, 0.0, r\z + 512.0 * RoomScale, 0, r, False)
			r\RoomDoors[1]\AutoClose = False : r\RoomDoors[1]\open = False
			
			r\Objects[0] = CreatePivot(r\obj)
			PositionEntity (r\Objects[0], r\x + 0 *RoomScale, 0.0, r\z-120*RoomScale, True)
			
			r\Objects[1] = CreatePivot(r\obj)
			PositionEntity (r\Objects[1], r\x + 0 *RoomScale, 0.0, r\z+120*RoomScale, True)
			
			r\Objects[2] = CreatePivot(r\obj)
			PositionEntity (r\Objects[2], r\x + 800 *RoomScale, -96 * RoomScale, r\z+0*RoomScale, True)
			
			r\Objects[3] = CreatePivot(r\obj)
			PositionEntity (r\Objects[3], r\x + 352 *RoomScale, -846 * RoomScale, r\z-576*RoomScale, True)
			
			it = CreateItem("Hastily Scribbled Note", "paper", r\x + 800 *RoomScale, 0, r\z+0*RoomScale)
			EntityParent(it\collider, r\obj)
			
			For n% = 4 To 7
			
				lBase = CopyEntity(LeverBaseOBJ)
				r\Objects[n] = CopyEntity(LeverOBJ)
			
				r\Levers[n-4] = r\Objects[n]
			
				ScaleEntity(lBase, 0.04, 0.04, 0.04)
				ScaleEntity(r\Objects[n], 0.04, 0.04, 0.04)
				
				If n=4 Then
					PositionEntity (lBase, r\x +572 * RoomScale, r\y -736 * RoomScale, r\z -1300 * RoomScale, True)
					PositionEntity (r\Objects[n], r\x +572 * RoomScale, r\y -736 * RoomScale, r\z -1300 * RoomScale, True)
					RotateEntity(lBase, 0, -90, 0)
					RotateEntity(r\Objects[n], 0, 90, 0)
				Else If n=5
					PositionEntity (lBase, r\x -1856 * RoomScale, r\y -736 * RoomScale, r\z +256 * RoomScale, True)
					PositionEntity (r\Objects[n], r\x -1856 * RoomScale, r\y -736 * RoomScale, r\z +256 * RoomScale, True)
					RotateEntity(lBase, 0, -90, 0)
					RotateEntity(r\Objects[n], 0, 90, 0)
				Else If n=6
					PositionEntity (lBase, r\x -516 * RoomScale, r\y -736 * RoomScale, r\z +776 * RoomScale, True)
					PositionEntity (r\Objects[n], r\x -516 * RoomScale, r\y -736 * RoomScale, r\z +776 * RoomScale, True)
					RotateEntity(lBase, 0, 90, 0)
					RotateEntity(r\Objects[n], 0, -90, 0)
				Else If n=7
					PositionEntity (lBase, r\x +1920 * RoomScale, r\y -736 * RoomScale, r\z +1280 * RoomScale, True)
					PositionEntity (r\Objects[n], r\x +1920 * RoomScale, r\y -736 * RoomScale, r\z +1280 * RoomScale, True)
					RotateEntity(lBase, 0, 90, 0)
					RotateEntity(r\Objects[n], 0, -90, 0)
				End If
				
				;RotateEntity(lBase, 0, 90, 0)
				;RotateEntity(r\Objects[(n * 2)+1], 10, 90, 0)
				
				EntityParent(lBase, r\obj)
				EntityParent(r\Objects[n], r\obj)
							
				EntityPickMode r\Objects[n], 1, False
				EntityRadius r\Objects[n], 0.1
		...
		Case "room2video"
			;[Block]
			r\RoomDoors[0] = CreateDoor(r\zone, r\x + 432.0 * RoomScale, 0, r\z, 270, r)
			r\RoomDoors[1] = CreateDoor(r\zone, r\x + 432.0 * RoomScale, -608 * RoomScale, r\z + 768 * RoomScale, 270, r)
			r\RoomDoors[2] = CreateDoor(r\zone, r\x + 16.0 * RoomScale, -608 * RoomScale, r\z + 336 * RoomScale, 270, r)
			r\RoomDoors[2]\locked = True
			r\RoomDoors[3] = CreateDoor(r\zone, r\x + 16.0 * RoomScale, -608 * RoomScale, r\z - 96 * RoomScale, 270, r)
			r\RoomDoors[3]\locked = True
			r\RoomDoors[4] = CreateDoor(r\zone, r\x + 16.0 * RoomScale, -608 * RoomScale, r\z - 528 * RoomScale, 270, r, False, False, 3)
			r\RoomDoors[4]\locked = False
			PositionEntity(r\RoomDoors[4]\buttons[1], r\x + 32.0 * RoomScale, EntityY(r\RoomDoors[4]\buttons[1],True), r\z -400.0 * RoomScale, True)
			r\RoomDoors[5] = CreateDoor(r\zone, r\x + 432.0 * RoomScale, -608 * RoomScale, r\z + 336 * RoomScale, 270, r)
			r\RoomDoors[5]\locked = True
			r\RoomDoors[6] = CreateDoor(r\zone, r\x + 432.0 * RoomScale, -608 * RoomScale, r\z - 96 * RoomScale, 270, r)
			r\RoomDoors[6]\locked = True
			r\RoomDoors[7] = CreateDoor(r\zone, r\x + 432.0 * RoomScale, -608 * RoomScale, r\z - 528 * RoomScale, 270, r)
			r\RoomDoors[7]\locked = True
			d.Doors = CreateDoor(r\zone, r\x + 224.0 * RoomScale, -608 * RoomScale, r\z - 880 * RoomScale, 0, r)
			d\locked = True
			
			it = CreateItem("Betamax Tape", "1981", r\x -304.0 * RoomScale, r\y - 432.0 * RoomScale, r\z - 608.0 * RoomScale)
			EntityParent(it\collider, r\obj)
			;[End Block]
		...
		Case "room2shaft"
			...
			r\Objects[2] = CreateElevButton(r\x + 1181.0 *RoomScale, r\y + 180.0 * RoomScale, r\z - 512.0 * RoomScale, 0, 270)
            EntityParent (r\Objects[2],r\obj)
			;[End Block]
		Case "room2poffices"
			;[Block]
		...
		Case "room2elevator"
			...
			r\RoomDoors[0] = CreateDoor(r\zone, r\x + 448.0 * RoomScale, 0.0, r\z, 90, r, False, 3, False, "", False, True)
		...
		Case "room2nuke"
			...
			r\RoomDoors[0] = CreateDoor(r\zone, r\x + 1192.0 * RoomScale, 0.0, r\z, 90, r, True, 3, False, "", False, True)
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = True
			;yl�kerran hissi
			r\Objects[4] = CreatePivot()
			...
			r\RoomDoors[1] = CreateDoor(r\zone, r\x + 680.0 * RoomScale, 1504.0 * RoomScale, r\z, 90, r, False, 3, False, "", False, True)
			r\RoomDoors[1]\AutoClose = False : r\RoomDoors[1]\open = False
			;alakerran hissi
			r\Objects[5] = CreatePivot()
...
;			
			...
			EntityParent(r\Objects[4], r\obj)
;			
			r\RoomDoors[0] = CreateDoor(r\zone, r\x + 264.0 * RoomScale, 0.0, r\z + 656.0 * RoomScale, 90, r, True, 3, False, "", False, True)
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = True
			...
			r\RoomDoors[2] = CreateDoor(r\zone, r\x - 264.0 * RoomScale, 0.0, r\z - 656.0 * RoomScale, 90, r, True, 3, False, "", False, True)
			r\RoomDoors[2]\AutoClose = False : r\RoomDoors[2]\open = True
...
;			
		...
		Case "room3storage"
			...
			;Doors
			r\RoomDoors[0] = CreateDoor(r\zone, r\x, 0.0, r\z + 448.0 * RoomScale, 0, r, True, 3, False, "", False, True)
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = True
			...
			r\RoomDoors[1] = CreateDoor(r\zone, r\x + 5840.0 * RoomScale,  -5632.0 * RoomScale, r\z + 1048.0 * RoomScale, 0, r, False, 3, False, "", False, True)
			r\RoomDoors[1]\AutoClose = False : r\RoomDoors[1]\open = False
			...
			r\RoomDoors[2] = CreateDoor(r\zone, r\x + 608.0 * RoomScale, 0.0, r\z - 312.0 * RoomScale, 0, r, True, 3, False, "", False, True)
			r\RoomDoors[2]\AutoClose = False : r\RoomDoors[2]\open = True
			...
			r\RoomDoors[3] = CreateDoor(r\zone, r\x - 456.0 * RoomScale,  -5632.0 * RoomScale, r\z - 824.0 * RoomScale, 0, r, False, 3, False, "", False, True)
			r\RoomDoors[3]\AutoClose = False : r\RoomDoors[3]\open = False
			;X=+176 | Z=-40
			...
			it = CreateItem("Scramble Goggles", "scrabble", r\x + 1936.0 * RoomScale, r\y - 5496.0 * RoomScale, r\z - 944.0 * RoomScale)
			EntityParent(it\collider, r\obj)
			it\state = 0
			
			it = CreateItem("Night Vision Goggles", "nvgoggles", r\x + 1946.0 * RoomScale, r\y - 5596.0 * RoomScale, r\z - 934.0 * RoomScale)
			EntityParent(it\collider, r\obj)
			it\state = 450
		...
		Case "room049"
			...
			r\RoomDoors[0] = CreateDoor(r\zone, r\x + 330.0 * RoomScale, 0.0, r\z + 656.0 * RoomScale, 90, r, True, 3, False, "", False, True)
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = True
			...
			r\RoomDoors[1] = CreateDoor(r\zone, r\x + 2898.0 * RoomScale, -3520.0 * RoomScale, r\z + 1824.0 * RoomScale, 90, r, False, 3, False, "", False, True)
			r\RoomDoors[1]\AutoClose = False : r\RoomDoors[1]\open = False	
			...
			r\RoomDoors[2] = CreateDoor(r\zone, r\x - 672.0 * RoomScale, 0.0, r\z - 408.0 * RoomScale, 0, r, True, 3, False, "", False, True)
			r\RoomDoors[2]\AutoClose = False : r\RoomDoors[2]\open = True
			...
			r\RoomDoors[3] = CreateDoor(r\zone, r\x - 2766.0 * RoomScale, -3520.0 * RoomScale, r\z - 1592.0 * RoomScale, 0, r, False, 3, False, "", False, True)
			r\RoomDoors[3]\AutoClose = False : r\RoomDoors[3]\open = False		
		...
		Case "room009"
			;[Block]
			r\RoomDoors[0] = CreateDoor(r\zone, r\x + 0 * RoomScale, 0.0 * RoomScale, r\z + 0 * RoomScale, 0, r, False, 2, 7, "", False)
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = False
			
			r\RoomDoors[1] = CreateDoor(r\zone, r\x -320 * RoomScale, 0.0 * RoomScale, r\z -656 * RoomScale, 90, r, False, 3, False, "", False, True)
			r\RoomDoors[1]\AutoClose = False : r\RoomDoors[1]\open = False
			
			r\RoomDoors[2] = CreateDoor(r\zone, r\x +320 * RoomScale, 0.0 * RoomScale, r\z +656 * RoomScale, 90, r, False, 3, False, "", False, True)
			r\RoomDoors[2]\AutoClose = False : r\RoomDoors[2]\open = False
			
			r\RoomDoors[3] = CreateDoor(r\zone, r\x -1024 * RoomScale, -4896 * RoomScale, r\z + 0 * RoomScale, 90, r, False, 3, False, "", False, True)
			r\RoomDoors[3]\AutoClose = False : r\RoomDoors[3]\open = True
			
			r\RoomDoors[4] = CreateDoor(r\zone, r\x +1024 * RoomScale, -4896 * RoomScale, r\z + 0 * RoomScale, 90, r, False, 3, False, "", False, True)
			r\RoomDoors[4]\AutoClose = False : r\RoomDoors[4]\open = True
			
			d = CreateDoor(r\zone, r\x + 0 * RoomScale, -4896 * RoomScale, r\z + 1024 * RoomScale, 0, r, False, 2, False, "", False)
			d\AutoClose = False : d\open = False : d\locked = True
			
			d = CreateDoor(r\zone, r\x + 0 * RoomScale, -4896 * RoomScale, r\z - 1024 * RoomScale, 0, r, False, 2, False, "", False)
			d\AutoClose = False : d\open = False : d\locked = True
			
			d = CreateDoor(r\zone, r\x + 0 * RoomScale, -4896 * RoomScale, r\z + 608 * RoomScale, 0, r, False, 2, False, "", False)
			d\AutoClose = False : d\open = False
			
			d = CreateDoor(r\zone, r\x + 0 * RoomScale, -4896 * RoomScale, r\z - 608 * RoomScale, 0, r, False, 2, False, "", False)
			d\AutoClose = False : d\open = True
			
			d = CreateDoor(r\zone, r\x - 608 * RoomScale, -4896 * RoomScale, r\z + 0 * RoomScale, 90, r, False, 2, False, "", False)
			d\AutoClose = False : d\open = False
			
			d = CreateDoor(r\zone, r\x + 800 * RoomScale, -4896 * RoomScale, r\z + 800 * RoomScale, 45, r, False, 2, False, "", False)
			d\AutoClose = False : d\open = True
			...
			d = CreateDoor(r\zone, r\x - 1024 * RoomScale, -4896 * RoomScale, r\z - 624 * RoomScale, 90, r, False, False, False, "", False)
			d\AutoClose = False : d\open = False : d\locked = True
			
			r\Objects[0] = CreatePivot(r\obj)
			PositionEntity(r\Objects[0], r\x + 576.0 * RoomScale, 0.0 * RoomScale, r\z + 656.0 * RoomScale, True)
			EntityParent(r\Objects[0], r\obj)
			
			r\Objects[1] = CreatePivot(r\obj)
			PositionEntity(r\Objects[1], r\x + 1332.0 * RoomScale, - 4860.0 * RoomScale, r\z + 0.0 * RoomScale, True)
			EntityParent(r\Objects[1], r\obj)
			
			r\Objects[2] = CreatePivot(r\obj)
			PositionEntity(r\Objects[2], r\x - 576.0 * RoomScale, 0.0 * RoomScale, r\z - 656.0 * RoomScale, True)
			EntityParent(r\Objects[2], r\obj)
			
			r\Objects[3] = CreatePivot(r\obj)
			PositionEntity(r\Objects[3], r\x - 1332.0 * RoomScale, - 4860.0 * RoomScale, r\z + 0.0 * RoomScale, True)
			EntityParent(r\Objects[3], r\obj)
			
			;Alarms
			
			r\Objects[4] = CreatePivot(r\obj)
			PositionEntity(r\Objects[4], r\x + 0.0 * RoomScale, 420.0 * RoomScale, r\z + 0.0 * RoomScale, True)
			EntityParent(r\Objects[4], r\obj)
			;[End Block]
		Case "room2_2"
			;[Block]
...
;			PositionEntity(r\Objects[6], r\x - 848*RoomScale, 0.5, r\z-576*RoomScale, True)
		...
		Case "room2closets"
			...
			d.Doors = CreateDoor(r\zone, r\x + 280.0 * RoomScale, 0.0*RoomScale, r\z + 576.0 * RoomScale, 90, r, False, False, -6)
			d\AutoClose = False
			d\MTFClose = False
			d\locked = False
			
			PositionEntity(d\buttons[0], r\x + 336.0 * RoomScale, EntityY(d\buttons[0],True), EntityZ(d\buttons[0],True), True)
		...
		Case "start"
			...
			d.Doors = CreateDoor(r\zone, r\x + 1392.0 * RoomScale, 384.0*RoomScale, r\z + 64.0 * RoomScale, 90, r, False, False, -6)
			d\AutoClose = False
			d\MTFClose = False
			d\locked = False
			...
			;r\Objects[5] = LoadSirenMesh("GFX\map\room1start.sirenmesh")
			;r\Objects[5] = LoadMesh_Strict("GFX\map\room1start_sirenmesh.b3d")
			;EntityPickMode r\Objects[5],2
			;EntityType r\Objects[5],HIT_MAP
			;EntityFX r\Objects[5],2
			;LightMesh r\Objects[5],-120,-120,-120
			;PositionEntity r\Objects[5],r\x + 0.0 * RoomScale,r\y + 0.0 * RoomScale,r\z + 0.0 * RoomScale,True
			;ScaleEntity r\Objects[6],RoomScale,RoomScale,RoomScale
			;Local tempTex% = LoadTexture_Strict("GFX\map\room1start.sirenmesh_lm.png")
			;TextureBlend tempTex,3
			;EntityParent r\Objects[5],r\obj
			
			r\Objects[6] = LoadAnimMesh_Strict("GFX\map\animated_alarm_siren.b3d")
			ScaleEntity r\Objects[6],RoomScale*46,RoomScale*46,RoomScale*46
			PositionEntity r\Objects[6],r\x+2057*RoomScale,r\y+1088*RoomScale,r\z+128*RoomScale,True
			EntityParent r\Objects[6],r\obj
			
			r\SpinningAlarmRotor[0] = r\Objects[6]
			r\SpinningAlarmLight[0] = CreateLight(3,r\Objects[6])
			PositionEntity r\SpinningAlarmLight[0],r\x+2057*RoomScale,r\y+1050*RoomScale,r\z+128.001*RoomScale,True
			LightRange r\SpinningAlarmLight[0],1.5
			LightColor r\SpinningAlarmLight[0],255*3,0,0
			...
			ScaleEntity(r\Objects[7], (204.0 * RoomScale) / MeshWidth(r\Objects[7]), 312.0 * RoomScale / MeshHeight(r\Objects[7]), 16.0 * RoomScale / MeshDepth(r\Objects[7]))
			EntityType r\Objects[7], HIT_MAP
			PositionEntity r\Objects[7], r\x + 0.0 * RoomScale, 0 * RoomScale, r\z + 1184.0 * RoomScale
			EntityParent(r\Objects[7], r\obj)
			
			r\Objects[8] = CopyEntity(DoorOBJ)
			ScaleEntity(r\Objects[8], (204.0 * RoomScale) / MeshWidth(r\Objects[8]), 312.0 * RoomScale / MeshHeight(r\Objects[8]), 16.0 * RoomScale / MeshDepth(r\Objects[8]))
			EntityType r\Objects[8], HIT_MAP
			PositionEntity r\Objects[8], r\x + 0.0 * RoomScale, 0 * RoomScale, r\z + 1184.0 * RoomScale
			EntityParent(r\Objects[8], r\obj)
			MoveEntity r\Objects[8],120.0,0,5.0
			
			r\Objects[9] = CopyEntity(DoorOBJ)
			ScaleEntity(r\Objects[9], (204.0 * RoomScale) / MeshWidth(r\Objects[9]), 312.0 * RoomScale / MeshHeight(r\Objects[9]), 16.0 * RoomScale / MeshDepth(r\Objects[9]))
			EntityType r\Objects[9], HIT_MAP
			PositionEntity r\Objects[9], r\x + 0.0 * RoomScale, 0 * RoomScale, r\z + 1184.0 * RoomScale
			RotateEntity(r\Objects[9], 0, 180, 0)
			EntityParent(r\Objects[9], r\obj)
			MoveEntity r\Objects[9],100.0,0,5.0
			...
			it = CreateItem("Janitor Key Card", "key0", r\x + 1748.0 * RoomScale, r\y + 768.0 * RoomScale, r\z + 9.0 * RoomScale)
			EntityParent(it\collider, r\obj)
		...
		Case "room018"
			;[Block]
			r\RoomDoors[0] = CreateDoor(r\zone, r\x, 0, r\z - 448.0 * RoomScale, 0, r, False, True, 3)
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = False
			PositionEntity(r\RoomDoors[0]\buttons[0], r\x - 384.0 * RoomScale, 0.7, r\z - 280.0 * RoomScale, True)
			
			r\RoomDoors[1] = CreateDoor(r\zone, r\x-400 * RoomScale, 0 * RoomScale, r\z + 0 * RoomScale, 90, r, False, False, 3)
			r\RoomDoors[1]\AutoClose = False : r\RoomDoors[1]\open = False
			
			r\RoomDoors[2] = CreateDoor(r\zone, r\x-816 * RoomScale, 0 * RoomScale, r\z + 112.0 * RoomScale, 0, r, False, False, 3)
			r\RoomDoors[2]\AutoClose = False : r\RoomDoors[2]\open = False
			PositionEntity(r\RoomDoors[2]\buttons[1], r\x - 912.0 * RoomScale, 0.7, r\z + 216.0 * RoomScale, True)
			TurnEntity(r\RoomDoors[2]\buttons[1], 0, -90, 0)
			
			r\RoomDoors[3] = CreateDoor(r\zone, r\x-816 * RoomScale, 0 * RoomScale, r\z + 1808.0 * RoomScale, 0, r, False, False, 3)
			r\RoomDoors[3]\AutoClose = False : r\RoomDoors[3]\open = False
			PositionEntity(r\RoomDoors[3]\buttons[0], r\x - 720.0 * RoomScale, 0.7, r\z + 1684.0 * RoomScale, True)
			TurnEntity(r\RoomDoors[3]\buttons[0], 0, -90, 0)
			PositionEntity(r\RoomDoors[3]\buttons[1], r\x -912.0 * RoomScale, 0.7, r\z + 1984.0 * RoomScale, True)
			TurnEntity(r\RoomDoors[3]\buttons[1], 0, -90, 0)
			
			r\RoomDoors[4] = CreateDoor(r\zone, r\x + 0 * RoomScale, -1536 * RoomScale, r\z + 368.0 * RoomScale, 0, r, False, False, 3)
			r\RoomDoors[4]\AutoClose = False : r\RoomDoors[4]\open = True : r\RoomDoors[4]\locked = True
			
			r\RoomDoors[5] = CreateDoor(r\zone, r\x + 0 * RoomScale, -1536 * RoomScale, r\z + 1808.0 * RoomScale, 180, r, False, False, 3)
			r\RoomDoors[5]\AutoClose = False : r\RoomDoors[5]\open = True : r\RoomDoors[5]\locked = True
			
			sc.SecurityCams = CreateSecurityCam(r\x - 320.0 * RoomScale, r\y + 704 * RoomScale, r\z + 288.0 * RoomScale, r, True)
			sc\angle = 45 + 180
			sc\turn = 45
			If Rand(0,2)=2 Then sc\FurryEffect=True
			;sc\CoffinEffect = True
			TurnEntity(sc\CameraObj, 120, 0, 0)
			EntityParent(sc\obj, r\obj)
			...
			r\Objects[4] = CreatePivot()
			PositionEntity(r\Objects[4], r\x, 328.0 * RoomScale, r\z - 908.0 * RoomScale)
			EntityParent(r\Objects[4], r\obj)
			
			r\Objects[5] = CreatePivot()
			PositionEntity(r\Objects[5], r\x - 848 * RoomScale, 368.0 * RoomScale, r\z - 294.0 * RoomScale)
			EntityParent(r\Objects[5], r\obj)
			
			r\Objects[6] = CreatePivot()
			PositionEntity(r\Objects[6], r\x - 848 * RoomScale, 268.0 * RoomScale, r\z + 2446.0 * RoomScale)
			EntityParent(r\Objects[6], r\obj)
			
			it = CreateItem("Document SCP-018", "paper", r\x - 688.0 * RoomScale, r\y + 133.0 * RoomScale, r\z - 304.0 * RoomScale)
			EntityParent(it\collider, r\obj)
			
			it = CreateItem("Level 2 Key Card", "key2", r\x + 240.0 * RoomScale, r\y -1456.0 * RoomScale, r\z + 2064.0 * RoomScale)
			EntityParent(it\collider, r\obj)
			
			it = CreateItem("Scramble Goggles", "scrabble", r\x + 280.0 * RoomScale, r\y -1456.0 * RoomScale, r\z + 2164.0 * RoomScale)
			EntityParent(it\collider, r\obj)
			
			it = CreateItem("Document SCP-3312", "paper", r\x + 280.0 * RoomScale, r\y -1456.0 * RoomScale, r\z + 2164.0 * RoomScale)
			EntityParent(it\collider, r\obj)
			...
			r\Objects[7] = CopyEntity(DoorOBJ)
			ScaleEntity(r\Objects[7], (204.0 * RoomScale) / MeshWidth(r\Objects[7]), 312.0 * RoomScale / MeshHeight(r\Objects[7]), 16.0 * RoomScale / MeshDepth(r\Objects[7]))
			EntityType r\Objects[7], HIT_MAP
			PositionEntity r\Objects[7], r\x + 0.0 * RoomScale, -1536.0 * RoomScale, r\z + 1808.0 * RoomScale
			RotateEntity(r\Objects[7], 0, 180 + 180, 0)
			EntityParent(r\Objects[7], r\obj)
			MoveEntity r\Objects[7],120.0,0,5.0
		...
		Case "914"
			...
			it = CreateItem("Scribbled Sticky Note", "paper", r\x + 0.0 * RoomScale, r\y + 112.0 * RoomScale, r\z + 0.0 * RoomScale)
			EntityParent(it\collider, r\obj)
		...
		Case "room178"
			;[Block]
			r\RoomDoors[0] = CreateDoor(r\zone, EntityX(r\obj) + 0.0 * RoomScale, 0, EntityZ(r\obj) -672.0 * RoomScale, 0, r, False, False, 2)
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\locked = False
			r\RoomDoors[0]\open = False
			
			r\Objects[0] = CreatePivot()
			PositionEntity (r\Objects[0], r\x + 0.0 * RoomScale, -448.0 * RoomScale, r\z -80 * RoomScale, True)
			EntityParent(r\Objects[0], r\obj)
			
			it = CreateItem("SCP-178","scp178",r\x,200.0 * RoomScale,r\z - 80.0 * RoomScale)
			RotateEntity it\collider,0,0,0
			EntityParent it\collider,r\obj
			it = CreateItem("Document SCP-3312","paper",r\x,200.0 * RoomScale,r\z - 320.0 * RoomScale)
			RotateEntity it\collider,0,0,0
			EntityParent it\collider,r\obj
			;SetAnimTime it\collider,0.0
			;[End Block]
		Case "173"
			;[Block]
			r\Objects[0] = CreatePivot()
			...
			r\Objects[6] = LoadAnimMesh_Strict("GFX\map\props\intro_sink.b3d")
			PositionEntity (r\Objects[6], EntityX(r\obj) -3900.0 * RoomScale, 88.0 * RoomScale, EntityZ(r\obj) + 289.0 * RoomScale)
			TurnEntity(r\Objects[6], 0, 180, 0)
			ScaleEntity(r\Objects[6], RoomScale, RoomScale, RoomScale)
			
			r\Objects[7] = LoadAnimMesh_Strict("GFX\map\props\intro_toilet.b3d")
			PositionEntity (r\Objects[7], EntityX(r\obj) -3900.0 * RoomScale, 88.0 * RoomScale, EntityZ(r\obj) + 289.0 * RoomScale)
			TurnEntity(r\Objects[7], 0, 180, 0)
			ScaleEntity(r\Objects[7], RoomScale, RoomScale, RoomScale)
			...
			r\RoomDoors[3] = CreateDoor(r\zone, r\x - 2320.0 * RoomScale, 0, r\z - 1248.0 * RoomScale, 90, r, True, False, -6)
			...
			;the elevator door
			r\RoomDoors[0] = CreateDoor(r\zone, r\x - 8288 * RoomScale, 0, r\z - 2272 * RoomScale, 0, r, False, 3, False, "", False, True)
			r\RoomDoors[0]\AutoClose = False
			...
			d.Doors = CreateDoor(r\zone, r\x - 5856 * RoomScale, 0, r\z - 1504 * RoomScale, 0, r, False, False, False, "", False, False, True)
			d\locked = True : d\DisableWaypoint = True
			...
			it = CreateItem("Class D Orientation Leaflet - Page 1/2", "paper", r\x-(2914+1024)*RoomScale, 170.0*RoomScale, r\z+40*RoomScale)
			EntityParent(it\collider, r\obj)
			
			it = CreateItem("Class D Orientation Leaflet - Page 2/2", "paper", r\x-(2914+1024)*RoomScale, 170.0*RoomScale, r\z+40*RoomScale)
			EntityParent(it\collider, r\obj)
			...
			;Local templight = CreateLight(2)
			;LightColor templight, 255, 255, 255
			;LightRange templight, 400
			;PositionEntity(templight, r\x - 3975 * RoomScale, r\y + 335.956 * RoomScale, r\z + 767.956 * RoomScale)
			;Cast_Shadow(r\obj)
			;Cast_Shadow(r\RoomDoors[6]\obj)
			;Cast_Shadow(r\RoomDoors[6]\frameobj)
			;Receive_Shadow(r\obj)
			
			;Cast_Light(templight, 400)
			;[End Block]
		Case "room2ccont"
			;[Block]
			r\RoomDoors[1] = CreateDoor(r\zone, r\x + 64.0 * RoomScale, 0.0, r\z + 368.0 * RoomScale, 180, r, False, False, False, "2975")
			r\RoomDoors[1]\AutoClose = False : r\RoomDoors[1]\open = False
			...
			RotateEntity(r\Objects[1], CurveValue(-85, EntityPitch(r\Objects[1]), 5), EntityYaw(r\Objects[1]), 0)
			RotateEntity(r\Objects[3], CurveValue(85, EntityPitch(r\Objects[3]), 5), EntityYaw(r\Objects[3]), 0)
		...
		Case "room106"
			;[Block]
			it = CreateItem("Note From Dr. Hoth", "paper", r\x - 752.0 * RoomScale, r\y - 592 * RoomScale, r\z + 3026.0 * RoomScale)
			EntityParent(it\collider, r\obj)
		...
		Case "room1lifts"
			;[Block]
			r\Objects[0] = CreateElevButton(r\x + 96.0*RoomScale, r\y + 160.0 * RoomScale, r\z + 64.0 * RoomScale, 0,0,0)
			EntityParent (r\Objects[0],r\obj)
			r\Objects[1] = CreateElevButton(r\x - 96.0*RoomScale, r\y + 160.0 * RoomScale, r\z + 64.0 * RoomScale, 0,0,0)
			EntityParent (r\Objects[1],r\obj)
...
Function InitWayPoints(loadingstart=45)
	...
	For w.WayPoints = Each WayPoints
		...
		iter = iter + 1
		If iter < 10 Then
			LoadingWhatAsset = "Waypoints - iter: 0"+iter+" number: "+number
		Else
			LoadingWhatAsset = "Waypoints - iter: "+iter+" number: "+number
		EndIf
		UpdateLoading() ;think of better way later
		If iter = 20 Then 
...
Global CoffinCam.SecurityCams, CAM018
Type SecurityCams
...
Function UpdateSecurityCams()
	...
	;FurryEffect = False, not affected by 3312
	;FurryEffect = True, constantly affected by 3312
			...
			If close = True Then
				If sc\Screen Then
					...
					If BlinkTimer > - 5 And EntityInView(sc\ScrObj, Camera) Then
						If EntityVisible(Camera,sc\ScrObj) Then
							If (sc\CoffinEffect=1 Or sc\CoffinEffect=3) And (Not Wearing714) And (WearingHazmat<3) And (WearingGasMask<3) Then
								...
								EndIf
							EndIf
							If (sc\FurryEffect=1) And (Not Wearing714) Then
								If BlinkTimer > - 5
									Looking=Looking-FPSfactor
									DebugLog Looking
								EndIf
							EndIf
						EndIf
					...
					Else
						...
						EndIf
					EndIf
					If (sc\FurryEffect=1) Then
						EntityTexture(sc\ScrOverlay, MonitorTexture3312)
						If (Not Wearing714) Then
							...
							If sc\InSight Then
								...
								PointEntity(pvt2, sc\ScrObj)
							
								RotateEntity(Collider, EntityPitch(Collider), CurveAngle(EntityYaw(pvt2), EntityYaw(Collider), Min(Max(15000.0 / (-Looking), 20.0), 200.0)), 0)
							
								TurnEntity(pvt2, 90, 0, 0)
								user_camera_pitch = CurveAngle(EntityPitch(pvt2), user_camera_pitch + 90.0, Min(Max(15000.0 / (-Looking), 20.0), 200.0))
								user_camera_pitch=user_camera_pitch-90
								
								FreeEntity pvt2	
								If Infect3312 < 1 And Looking < -150 Then
									Infect3312 = 1
									InfectType3312=0
								EndIf
							Else
								If Looking < 0 Then
									Looking = Min(Looking + FPSfactor, 0.0)
								EndIf
							EndIf
						Else
							If sc\InSight Then
					...
					If sc\InSight And sc\CoffinEffect=0 Or sc\CoffinEffect=2 Then
						...
						If (sc\FurryEffect=1) Then EntityTexture(sc\ScrOverlay, MonitorTexture3312)
				...
				If Abs(EntityX(Collider)-EntityX(room1,True))<280.0*RoomScale+(0.015*FPSfactor) Then
					If Abs(EntityZ(Collider)-EntityZ(room1,True))<280.0*RoomScale+(0.015*FPSfactor) Then	
						If Abs(EntityY(Collider)-EntityY(room1,True))<280.0*RoomScale+(0.015*FPSfactor) Then	
							...
							If (Not ChannelPlaying(event\ElevSoundCHN)) Then
								event\ElevSoundCHN = PlaySound_Strict(ElevatorMoveSFX)
								DebugLog("Elevator is Moving, Check Sucessful")
								;room079happened = False
							Else
								DebugLog("Elevator is Moving, Check Failed")
								;If (Not ChannelPlaying(event\SoundCHN)) Then 
								;	;event\SoundCHN = 0
								;	event\SoundCHN = PlaySound_Strict(ElevatorMoveSFX)
								;Else
								;	event\SoundCHN2 = PlaySound_Strict(ElevatorMoveSFX)
								;EndIf
								;room079happened = False
							EndIf
...
Function CreateMap()
	...
	SetRoom("room2video", ROOM2, Floor(0.3*Float(Room2Amount[0])),min_pos,max_pos)
	...
	SetRoom("room2testpicture",ROOM2,Floor(0.85*Float(Room2Amount[0])),min_pos,max_pos)
	SetRoom("room2test",ROOM2,Floor(0.85*Float(Room2Amount[0])),min_pos,max_pos)
	...
	SetRoom("room018", ROOM1, Room1Amount[0]+Floor(0.10*Float(Room1Amount[1])),min_pos,max_pos)

	SetRoom("room096", ROOM1, Room1Amount[0]+Floor(0.13*Float(Room1Amount[1])),min_pos,max_pos)
	...
	SetRoom("room009", ROOM2, Room2Amount[0]+Floor(0.55*Float(Room2Amount[1])),min_pos,max_pos)
	...
	SetRoom("room2generators",ROOM2,Room2Amount[0]+Floor(0.8*Float(Room2Amount[1])),min_pos,max_pos)
						...
						If shouldSpawnDoor
							If (x+1)<(MapWidth+1)
								If MapTemp(x + 1, y) > 0 Then
									d.Doors = CreateDoor(r\zone, Float(x) * spacing + spacing / 2.0, 0, Float(y) * spacing, 90, r, Max(Rand(-3, 1), 0), temp, False, "", False, False, tempEntrance)
									r\AdjDoor[0] = d
								EndIf
							EndIf
						EndIf
						...
						If shouldSpawnDoor
							If (y+1)<(MapHeight+1)
								If MapTemp(x, y + 1) > 0 Then
									d.Doors = CreateDoor(r\zone, Float(x) * spacing, 0, Float(y) * spacing + spacing / 2.0, 0, r, Max(Rand(-3, 1), 0), temp, False, "", False, False, tempEntrance)
									r\AdjDoor[3] = d
								EndIf
							EndIf
						EndIf
...
Function UpdateAlarmRotor(rotorMdl%,lightCone%, status%=0)
	Local realStatus%
