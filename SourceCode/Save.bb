...
Function SaveGame(file$)
	...
	Local f% = WriteFile(file + "Data.dbsav")
	...
	WriteFloat f, Infect
	WriteFloat f, Infect3312
	WriteFloat f, InfectType3312
	WriteFloat f, InfectOverride3312
	...
	If Not MenuOpen Then
		...
		;MsgTimer = 70 * 4
		SetSubMSG("Game progress saved.",4)
	EndIf
...
Function LoadGame(file$)
	...
	If FileSize(file + "save.txt") = 0 Then
		f% = ReadFile(file + "Data.dbsav")
	Else
		f% = ReadFile(file + "save.txt")
	EndIf
	...
	Infect = ReadFloat(f)
	Infect3312 = ReadFloat(f)
	InfectType3312 = ReadFloat(f)
	InfectOverride3312 = ReadFloat(f)
	...
	;If version = CompatibleNumber Or version = VersionNumber Then
						...
						If shouldSpawnDoor
							If (x+1)<(MapWidth+1)
								If MapTemp(x + 1, y) > 0 Then
									do.Doors = CreateDoor(r\zone, Float(x) * spacing + spacing / 2.0, 0, Float(y) * spacing, 90, r, Max(Rand(-3, 1), 0), temp, False, "", False, False, tempEntrance)
									r\AdjDoor[0] = do
								EndIf
							EndIf
						EndIf
						...
						If shouldSpawnDoor
							If (y+1)<(MapHeight+1)
								If MapTemp(x, y + 1) > 0 Then
									do.Doors = CreateDoor(r\zone, Float(x) * spacing, 0, Float(y) * spacing + spacing / 2.0, 0, r, Max(Rand(-3, 1), 0), temp, False, "", False, False, tempEntrance)
									r\AdjDoor[3] = do
								EndIf
							EndIf
						EndIf
...
Function LoadSaveGames()
	...
	Repeat 
		...
		If FileType(SavePath+"\"+file$) = 2 Then 
			If file <> "." And file <> ".." Then 
				If (FileType(SavePath + file + "\save.txt")>0) Or (FileType(SavePath + file + "\Data.dbsav")>0) Then
	...
	For i = 1 To SaveGameAmount
		...
		If FileSize(SavePath + SaveGames(i - 1) + "\save.txt") = 0 Then
			DebugLog (SavePath + SaveGames(i - 1) + "\Data.dbsav")
			f% = ReadFile(SavePath + SaveGames(i - 1) + "\Data.dbsav")
		Else
			...
			If Not (SaveGameVersion(i - 1) = "2.2.9" Or SaveGameVersion(i - 1) = "2.2.8" Or SaveGameVersion(i - 1) = "2.2.7")
				If Not (SaveGameVersion(i - 1) = "2.2.6" Or SaveGameVersion(i - 1) = "2.2.5" Or SaveGameVersion(i - 1) = "2.2.4")
					If Not (SaveGameVersion(i - 1) = "2.2.3" Or SaveGameVersion(i - 1) = "2.2.2" Or SaveGameVersion(i - 1) = "2.2.1")
						SaveGameDataVersion(i - 1) = ReadFloat(f)
					EndIf
				EndIf
			EndIf
		EndIf
