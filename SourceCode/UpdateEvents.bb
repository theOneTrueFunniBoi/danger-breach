		...
		Select e\EventName
			...
			Case "alarm" ;the alarm in the starting room
				...
				If PlayerRoom = e\room Or IsRoomAdjacent(e\room,PlayerRoom) Then
					...
					ShowEntity e\room\SpinningAlarmLight[0]
				Else
					HideEntity e\room\SpinningAlarmRotor[0]
					HideEntity e\room\SpinningAlarmLight[0]
				EndIf
				If PlayerRoom = e\room Then
					If e\room\NPC[8] <> Null Then
						If e\room\NPC[8]\Frame = 999 Or e\room\NPC[8]\Frame < 285
							AnimateNPC(e\room\NPC[8], 249, 286, 0.4)
						Else
							AnimateNPC(e\room\NPC[8], 286, 286, 0.4)
						EndIf
					EndIf					
					;DebugLog "position of r\objects[5] is "+EntityX(e\room\Objects[5])+", "+EntityY(e\room\Objects[5])+", "+EntityZ(e\room\Objects[5])
					
					pvt% = CreatePivot()
					Local p_ent% = e\room\Objects[8]
					PositionEntity(pvt, EntityX(p_ent%,True), EntityY(p_ent%,True)+Rnd(0.0,0.05), EntityZ(p_ent%,True))
					RotateEntity(pvt, 0, EntityYaw(p_ent%,True)+90, 0)
				...
				If e\EventState = 0 Then
					If PlayerRoom = e\room Then
						...
						If e\room\NPC[8] = Null Then 
							e\room\NPC[8] = CreateNPC(NPCtypeGuard,0,0,0)
							e\room\NPC[8]\State = 8
						EndIf
						
						HideEntity e\room\NPC[8]\obj
						HideEntity e\room\NPC[8]\Collider
						...
						If SelectedDifficulty\otherFactors = NORMAL
							If Rand(0,12)=1 Then sc\FurryEffect=True
						ElseIf SelectedDifficulty\otherFactors = HARD
							If Rand(0,6)=1 Then sc\FurryEffect=True
						EndIf
							...
							If e\EventState > 900 And e\room\RoomDoors[5]\open Then
								...
								Else
									If e\EventState-FPSfactor < 900+4*70 Then 
										...
										PositionEntity Curr173\Collider, EntityX(e\room\NPC[2]\Collider), EntityY(e\room\NPC[2]\Collider), EntityZ(e\room\NPC[2]\Collider), True
									...
									If e\room\NPC[2]\State <> 1 And KillTimer >= 0
										...
										If EntityZ(e\room\NPC[2]\Collider) < e\room\z+12*RoomScale Then
											...
											PositionEntity(e\room\NPC[8]\Collider, EntityX(e\room\NPC[2]\Collider, True), EntityY(e\room\NPC[2]\Collider, True), EntityZ(e\room\NPC[2]\Collider, True))
											...
											SetNPCFrame(e\room\NPC[8], 999)
											
											ShowEntity e\room\NPC[8]\obj
											ShowEntity e\room\NPC[8]\Collider
											EntityType e\room\NPC[8]\Collider, HIT_DEAD
											
											;AnimateNPC(e\room\NPC[8], 285, 286, 0.4)
										EndIf
									EndIf
								EndIf
					...
					If e\EventState3>0 Then
						...
						If e\EventState3 < 170 Then
							If e\EventState3 = 1.0 Then
								...
								MusicCHN = StreamSound_Strict("SFX\Character\WakeUp\WakeUpEffect.ogg",CurrMusicVolume,Mode)
								...
								Msg = Chr(34)+"What a weird dream..."+Chr(34)
								MsgTimer=70*7
							EndIf
						...
						ElseIf e\EventState3 < 900
									...
									If e\room\RoomDoors[7]\open Then 
										If Distance(EntityX(Collider), EntityZ(Collider), EntityX(e\room\obj,True)-3328*RoomScale, EntityZ(e\room\obj,True)-1232*RoomScale)<5.0 Then
											...
											If e\EventStr = "done" Then 
												PlaySound_Strict LoadTempSound("SFX\Room\Intro\PA\scripted\announcement"+Rand(1,10)+".ogg")
							...
							If Distance(EntityX(e\room\NPC[3]\Collider), EntityZ(e\room\NPC[3]\Collider), EntityX(e\room\RoomDoors[2]\frameobj,True), EntityZ(e\room\RoomDoors[2]\frameobj,True)) < 4.5 And dist < 5.0 Then
								...
								PlaySound_Strict LoadTempSound("SFX\Door\DoorClose3.ogg")
						...
						ElseIf e\EventState3 <= 905
							...
							ElseIf e\room\NPC[3]\Frame >= 358.0 Then
								;PointEntity e\room\NPC[3]\Collider,Collider
								;RotateEntity e\room\NPC[3]\Collider,0,EntityYaw(e\room\NPC[3]\Collider),0
								e\room\NPC[3]\BoneToManipulate = "head"
								e\room\NPC[3]\ManipulateBone = True
								e\room\NPC[3]\ManipulationType = 0
								...
								Else
									...
									If (EntityDistance(Collider, e\room\NPC[3]\Collider)<1.5) Then
										If EntityInView(e\room\NPC[3]\Collider, Camera) Then
											DrawHandIcon = True
											
											If MouseHit1 Then
												SelectedItem = CreateItem("Document SCP-173 FOR D-CLASS PERSONNEL", "paper", 0.0, 0.0, 0.0)
												...
												;e\room\NPC[3]\State2 = 0
												e\room\NPC[3]\State = 8
												e\room\NPC[3]\Frame = 600
											EndIf
										EndIf
									EndIf
								EndIf
						...
						Else
							...
							If e\room\NPC[3]\State3 = 0 Then
								If e\room\NPC[3]\Frame <= 620.5 And e\room\NPC[3]\State = 8 Then
									AnimateNPC(e\room\NPC[3],608,621,0.4,False)
								Else
									e\room\NPC[3]\Sound2 = LoadSound_Strict("SFX\Room\Intro\Guard\Music"+Rand(1,5)+".ogg")
									e\room\NPC[3]\Angle = EntityYaw(e\room\NPC[3]\Collider)
									e\room\NPC[3]\State3 = 1
									e\room\NPC[3]\State = 20
									e\room\NPC[4]\State = 9
								EndIf
							Else								
								If e\room\NPC[3]\Frame < 712 And e\room\NPC[3]\Frame > 710
									e\room\NPC[3]\SoundChn2 = LoopSound2(e\room\NPC[3]\Sound2,e\room\NPC[3]\SoundChn2,Camera,e\room\NPC[3]\obj,2,0.5)
								EndIf
								...
								If e\room\RoomDoors[2]\open Then
									...
									If Distance(EntityX(Collider), EntityZ(Collider), EntityX(e\room\obj), EntityZ(e\room\obj)) < 4.0 Then
										...
										e\room\RoomDoors[2]\locked = True
										e\EventState3 = 0
										If e\room\NPC[3]\SoundChn2<>0 Then
											If ChannelPlaying(e\room\NPC[3]\SoundChn2) Then
												StopChannel(e\room\NPC[3]\SoundChn2)
												FreeSound_Strict e\room\NPC[3]\Sound2 
												e\room\NPC[3]\Sound2=0											
											EndIf
										EndIf
										e\room\NPC[3]\State = 24
										e\room\NPC[4]\State = 0
					...
					Else
						...
						If e\EventState = 0 Then
							If PlayerRoom = e\room Then
								...
								e\room\NPC[11] = CreateNPC(NPCtypeTrain, e\room\x-7264*RoomScale, -600*RoomScale, e\room\z-3064*RoomScale)
								e\room\NPC[11]\State = 1
								RotateEntity e\room\NPC[11]\Collider,0,e\room\angle-90,0
						...
						ElseIf e\EventState < 14000 ; player is inside the room
							...
							ElseIf e\EventState => 11561 And e\EventState - FPSfactor < 11561 ;lights go out
								e\EventState = 14000
								PlaySound_Strict LoadTempSound("SFX\Room\Intro\PA\scripted\announcement_breach.ogg")
			...
			Case "checkpoint"
				...
				If e\room\RoomTemplate\Name = "checkpoint2"
					If (e\room\RoomDoors[1]\open = True) Or (e\room\RoomDoors[0]\open = True) Then
						GiveAchievement(AchvEntrance)
						;If e\EventState4 = 0 Then PlaySound_Strict(LoadTempSound("SFX\Room\BlackoutShort.ogg"))
						;If (e\EventState4 = 0 Or e\EventState4 = 1) Then
						;	SecondaryLightOn = CurveValue(0, SecondaryLightOn, 10.0)
						;	e\EventState4 = 1
						;endIf
					EndIf
					FPStempThing=0 ;funni-ness
					If ((e\room\RoomDoors[1]\open = False) Or (e\room\RoomDoors[0]\open = False)) And (e\EventState4 = 1) Then e\EventState4 = 2
				...
				Else
					...
					Next
					If (e\room\RoomDoors[1]\open = True) Or (e\room\RoomDoors[0]\open = True) Then
						GiveAchievement(AchvHeavy)
					EndIf
					
					e\EventState4 = UpdateElevators(e\EventState4, e\room\RoomDoors[2], e\room\RoomDoors[3], e\room\Objects[4], e\room\Objects[5], e, False)
			...
			Case "room018"
				...
				If PlayerRoom = e\room Then
					...
					If e\room\RoomDoors[0]\open = True Then
						GiveAchievement(Achv018)
					EndIf
					If EntityPitch(e\room\Levers[0],True) > 0 Then
						e\SoundCHN = LoopSound2(AlarmSFX(3), e\SoundCHN, Camera, e\room\Objects[4], 5.0)
						e\SoundCHN2 = LoopSound2(AlarmSFX(3), e\SoundCHN2, Camera, e\room\Objects[5], 5.0)
						e\SoundCHN3 = LoopSound2(AlarmSFX(3), e\SoundCHN3, Camera, e\room\Objects[6], 5.0)
					Else
			...
			Case "room2ccont"
				...
				If PlayerRoom = e\room Then
					...
					;Primary Lighting
					e\EventState4 = UpdateLever(e\room\Objects[1])
					
					Local sparkSound% = 1
					
					If e\EventState4  And e\EventState = 1
						e\EventState = 2
						sparkSound = Rand(1,5)
						If e\Sound = 0 Then LoadEventSound(e,"SFX\Room\Room2ccont\FX_Spark_0"+sparkSound+".ogg")
					...
					Local prevstate2
					;If e\EventState5
						prevstate2 = e\EventState2
						e\EventState2 = UpdateLever(e\room\Objects[3])
					;Else
					;	e\EventState2 = 1.0
					;EndIf
					
					;minor hack - put this check after we update lever so we can override eventstate2
					;If e\EventState = 0 Then
					;	If e\room\RoomDoors[1]\open
					;		e\EventState = 1
					;		;RotateEntity(e\room\Objects[1], CurveValue(-85, EntityPitch(e\room\Objects[1]), 5), EntityYaw(e\room\Objects[1]), 0)
					;		RotateEntity(e\room\Objects[3], CurveValue(-85, EntityPitch(e\room\Objects[3]), 5), EntityYaw(e\room\Objects[3]), 0) ;why you comment this out dumbass					;		e\EventState2 = 0.0
					;		prevstate2 = 0.0
					...
					If e\EventState3 = 0 Then
						If (RemoteDoorOn=False) Then
							PlayAnnouncement("SFX\Character\MTF\AnnouncDoor1.ogg")
							e\EventState3 = 1
						EndIf
					EndIf
				EndIf
			...
			Case "room2tesla"
				...
				If temp And EntityY(Collider, True) > EntityY(e\room\obj,True) And EntityY(Collider, True) < 4.0 Then
					...
					If e\EventState5 > 0 Then e\EventState5 = e\EventState5-FPSfactor
					...
					If e\EventState = 0 Then
						If e\room\dist < 8 Then
							...
							For i = 0 To 2
								...
								If (Distance(EntityX(Curr173\Collider),EntityZ(Curr173\Collider),EntityX(e\room\Objects[i],True),EntityZ(e\room\Objects[i],True)) < 300.0*RoomScale) Then
									;play the activation sound
									If KillTimer => 0 Then 
										PlayerSoundVolume = Max(8.0,PlayerSoundVolume)
										StopChannel(e\SoundCHN)
							...
							For i = 0 To 2
								If Distance(EntityX(Curr106\Collider),EntityZ(Curr106\Collider),EntityX(e\room\Objects[i],True),EntityZ(e\room\Objects[i],True)) < 300.0*RoomScale Then
									...
									If KillTimer => 0 Then 
										...
										HideEntity e\room\Objects[4]
										e\EventState = 1
										e\EventState4 = 1;106 triggered the tesla gate
						...
						If (Not Curr049 = Null)
							If Curr049\State > 1 And e\EventState = 0 And e\EventState5 =< 0 Then 
								For i = 0 To 2
									If Distance(EntityX(Curr049\Collider),EntityZ(Curr049\Collider),EntityX(e\room\Objects[i],True),EntityZ(e\room\Objects[i],True)) < 300.0*RoomScale Then
										;play the activation sound
										If KillTimer => 0 Then 
											StopChannel(e\SoundCHN)
											e\SoundCHN = PlaySound2(TeslaActivateSFX, Camera, e\room\Objects[3],4.0,0.5)
											HideEntity e\room\Objects[4]
											e\EventState = 1
											e\EventState4 = 2;049 triggered the tesla gate
											e\EventState5 = 380
											Exit
										EndIf
									EndIf
						...
						If (Not CurrZombie = Null)
							For i = 0 To 2
								If (Distance(EntityX(CurrZombie\Collider),EntityZ(CurrZombie\Collider),EntityX(e\room\Objects[i],True),EntityZ(e\room\Objects[i],True)) < 300.0*RoomScale) And (Not CurrZombie\IsDead) Then
									;play the activation sound
									If KillTimer => 0 Then 
										PlayerSoundVolume = Max(8.0,PlayerSoundVolume)
										StopChannel(e\SoundCHN)
										e\SoundCHN = PlaySound2(TeslaActivateSFX, Camera, e\room\Objects[3],4.0,0.5)
										e\EventState = 1
										e\EventState4 = 3;049-2 triggered the tesla gate
										Exit
									EndIf
								EndIf
							Next
						...
						If (Not Curr096 = Null)
							If Curr096\State = 4 And e\EventState = 0 And e\EventState5 =< 0 Then 
								For i = 0 To 2
									If Distance(EntityX(Curr096\Collider),EntityZ(Curr096\Collider),EntityX(e\room\Objects[i],True),EntityZ(e\room\Objects[i],True)) < 300.0*RoomScale Then
										...
										If KillTimer => 0 Then 
											...
											HideEntity e\room\Objects[4]
											e\EventState = 30
											e\EventState4 = 4;096 triggered the tesla gate
											e\EventState5 = 480
											Exit
										EndIf
									EndIf
						...
						If (Not Curr008 = Null)
							For i = 0 To 2
								If (Distance(EntityX(Curr008\Collider),EntityZ(Curr008\Collider),EntityX(e\room\Objects[i],True),EntityZ(e\room\Objects[i],True)) < 300.0*RoomScale) And (Not Curr008\IsDead) Then
									;play the activation sound
									If KillTimer => 0 Then 
										PlayerSoundVolume = Max(8.0,PlayerSoundVolume)
										StopChannel(e\SoundCHN)
										e\SoundCHN = PlaySound2(TeslaActivateSFX, Camera, e\room\Objects[3],4.0,0.5)
										e\EventState = 1
										e\EventState4 = 5;008-2 triggered the tesla gate
										Exit
									EndIf
								EndIf
							Next
					...
					Else
						...
						Else
							...
							If e\EventState < 70 Then 
								...
								If e\EventState4 = 1 Then ;106 triggered the tesla gate
									;ShowEntity Light
									LightFlash = 0.3
									If ParticleAmount > 0
									...
									e\EventState4 = 0 ;reset for later
								ElseIf e\EventState4 = 2 Then ;049 triggered the tesla gate
									LightFlash = 0.3
									Curr049\Frame = 140
									Curr049\State = 1
									e\EventState4 = 0 ;reset for later
								ElseIf e\EventState4 = 3 Then ;049-2 triggered the tesla gate
									LightFlash = 0.3
									CurrZombie\IsDead = True
									e\EventState4 = 0 ;reset for later
								ElseIf e\EventState4 = 4 Then ;096 triggered the tesla gate
									LightFlash = 0.3
									;Curr096\Frame = 140
									Curr096\State2 = 1
									e\EventState4 = 0 ;reset for later
								ElseIf e\EventState4 = 5 Then ;008-2 triggered the tesla gate
									LightFlash = 0.3
									Curr008\IsDead = True
									e\EventState4 = 0 ;reset for later
								EndIf
...
;						EndIf
					...
					If temp = True Then 
						...
						Select e\EventState 
							...
							Case 52
								...
								EndIf
							Case 60
								If (Not HalloweenTex) Then ;enable halloween 173
									Local tex970 = LoadTexture_Strict("GFX\npcs\173h.pt", 1)
									EntityTexture Curr173\obj, tex970, 0, 0
									EntityTexture Curr173\obj3, tex970, 0, 0
									FreeTexture tex970
									halloween990=True
								EndIf
						End Select
			...
			Case "room2testpicture"
				...
				If PlayerRoom = e\room Then
					If (Not e\room\RoomDoors[0]\open)	
						...
						;n\Angle = WrapAngle(DeltaYaw(r\Objects[0]\Collider, Collider));-EntityYaw(n\Collider,True))
						;CurrCameraZoom = 10
						Msg = "You feel as though something is not right..."
						MsgTimer = 70 * 7
						n.NPCs = CreateNPC(NPCtype096, EntityX(e\room\Objects[1],True),0.5,EntityZ(e\room\Objects[1],True))
					EndIf
				EndIf
				;[End Block]
			Case "room2testmisc"
			...
			Case "room2generators"
				;[Block]
				;temporarily commented out voicelines because they are literal fucking cringe
				...
				If PlayerRoom = e\room Then
					If e\EventState = 0 Then
						If e\room\RoomDoors[1]\open = True
							e\room\RoomDoors[0]\locked = True
							;e\SoundCHN = PlaySound2(LoadTempSound("SFX\SCP\3312-1\victim1.ogg"),Camera,e\room\Objects[3])
							e\EventState = 1
						Else If e\room\RoomDoors[0]\open = True
							e\room\RoomDoors[1]\locked = True
							;e\SoundCHN = PlaySound2(LoadTempSound("SFX\SCP\3312-1\victim1.ogg"),Camera,e\room\Objects[3])
							e\EventState = 1
						EndIf
					Else If e\EventState = 1
						If (EntityDistance(e\room\Objects[0],Collider)<0.5) Or (EntityDistance(e\room\Objects[1],Collider)<0.5)
							e\room\RoomDoors[1]\locked = True
							e\room\RoomDoors[1]\open = False
							e\room\RoomDoors[0]\locked = True
							e\room\RoomDoors[0]\open = False
						...
						EndIf
					Else If e\EventState = 2
						If (EntityDistance(e\room\Objects[2],Collider)<0.5) 
							GiveAchievement(AchvFurry)
							;PlaySound_Strict(LoadTempSound("SFX\SCP\3312-1\hello.ogg"))
							e\EventState = 3
						EndIf
					Else If e\EventState = 3
						If (EntityDistance(e\room\Objects[3],Collider)<0.5) 
							;PlaySound_Strict(LoadTempSound("SFX\SCP\3312-1\laugh1.ogg"))
							e\EventState = 5
						EndIf
					Else If e\EventState = 4 Or e\EventState = 5
						;RotateEntity(e\room\Objects[5], 10, -180, 0)
						e\EventState2 = UpdateLever(e\room\Objects[4])
						e\EventState3 = UpdateLever(e\room\Objects[5])
						e\EventState4 = UpdateLever(e\room\Objects[6])
						e\EventState5 = UpdateLever(e\room\Objects[7])
						If e\EventState2 And e\EventState3 And e\EventState4 And e\EventState5
							ShouldPlay = Min(PlayerZone,2)
							SecondaryLightOn = CurveValue(1, SecondaryLightOn, 10.0)
						...
						Else
							...
							EndIf
							If ChannelPlaying(e\SoundCHN3) Then
					...
					Else If e\EventState = 6
						DeathMSG = "Subject D-9341 was found dead on the floor of the Site-[REDACTED] Heavy Containment Zone "
						DeathMSG = DeathMSG + "Generator-Cluster [REDACTED] maintanance area. Several lacerations were found on the body. The cause of "
						DeathMSG = DeathMSG + "the wounds is of yet unknown."
						Kill()	
					EndIf
				EndIf
				;[End Block]
...
;						EndIf
			...
			Case "room009"
				...
				If PlayerRoom = e\room Then
					...
					e\SoundCHN2 = LoopSound2(AlarmSFX(3), e\SoundCHN2, Camera, e\room\Objects[4], 5.0)
				
					e\EventState = UpdateElevators(e\EventState, e\room\RoomDoors[1], e\room\RoomDoors[3], e\room\Objects[2], e\room\Objects[3], e)
					
					e\EventState2 = UpdateElevators(e\EventState2, e\room\RoomDoors[2], e\room\RoomDoors[4], e\room\Objects[0], e\room\Objects[1], e)
			...
			Case "room079"
				...
				If PlayerRoom = e\room Then
					...
					;If room079ground = False And room079happened = False Then
					;	TurnEntity(e\room\Objects[4], 0,90,0,True)
					;	room079happened = True
					;ElseIf room079ground = True And room079happened = False
					;	TurnEntity(e\room\Objects[4], 0,-180,0,True)
					;	room079happened = True
					;EndIf
				
					e\EventState3 = UpdateElevators(e\EventState3, e\room\RoomDoors[1], e\room\RoomDoors[2], e\room\Objects[3], e\room\Objects[4], e)
					...
					ElseIf e\EventState < 10000
						...
						ElseIf e\EventState = 2	
							If EntityDistance(e\room\Objects[0], Collider)<3.0 Then 
								...
								Msg = Chr(34)+"I wonder what it's going to tell me..."+Chr(34)
								MsgTimer = 70 * 7
							EndIf							
				...
				If e\EventState2 = 1 Then
					If RemoteDoorOn Then 	
						...
						If e\SoundCHN<>0
							StopStream_Strict(e\SoundCHN) : e\SoundCHN=0
						EndIf
						e\SoundCHN = StreamSound_Strict("SFX\SCP\079\GateB"+Rand(1,3)+".ogg",SFXVolume,0)
						e\SoundCHN_isStream = True
						
						PlayAnnouncement("SFX\Character\MTF\AnnouncDoor2.ogg")
			...
			Case "room096"
				...
				If PlayerRoom = e\room Then
					If RemoteDoorOn Then
						If (e\EventState <> 1) And (e\EventState <> 2) Then
							e\room\RoomDoors[2]\open = False
							PlaySound_Strict (LoadTempSound("SFX\Door\DoorClose079.ogg"))
							e\EventState = 1
					
							e\room\NPC[0]=CreateNPC(NPCtypeGuard, EntityX(e\room\Objects[0],True), EntityY(e\room\Objects[0],True)+2, EntityZ(e\room\Objects[0],True))
							SetNPCFrame(e\room\NPC[0], 288)
							e\room\NPC[0]\State = 8
						EndIf
						If (e\room\RoomDoors[0]\open) And (e\EventState = 1) Then 
							If e\room\RoomDoors[0]\openstate > 50 Or EntityDistance(Collider, e\room\RoomDoors[0]\frameobj)<0.5 Then
								e\room\RoomDoors[0]\openstate = Min(e\room\RoomDoors[0]\openstate,50)
								e\room\RoomDoors[0]\open = False
								PlaySound_Strict (LoadTempSound("SFX\Door\DoorError.ogg"))
					...
					If e\EventState = 1 Then
						If EntityDistance(e\room\Objects[0], Collider)<3.0 Then
						;If e\room\RoomDoors[0]\open = True Then
							e\room\NPC[1]=CreateNPC(NPCtype096, EntityX(e\room\Objects[1],True), EntityY(e\room\Objects[1],True)+2, EntityZ(e\room\Objects[1],True))
							e\EventState = 2
						EndIf
					EndIf
				EndIf
			...
			Case "room178"
				;[Block]
				If (PlayerRoom = e\room) And (e\EventState <> 1) And (e\room\RoomDoors[0]\open = False) Then
					e\room\RoomDoors[0]\open = True
					;e\room\RoomDoors[0]\locked = True
					PlaySound_Strict LoadTempSound("SFX\Door\DoorOpen079.ogg")
					e\EventState = 1
					;Msg = Chr(34)+"Where are the glasses?"+Chr(34)
					;MsgTimer = 70 * 7
					;RemoveNPC(NPCtype173)
					e\room\NPC[0]=CreateNPC(NPCtypeGuard, EntityX(e\room\Objects[0],True), EntityY(e\room\Objects[0],True)+2, EntityZ(e\room\Objects[0],True))
					SetNPCFrame(e\room\NPC[0], 288)
					e\room\NPC[0]\State = 8
				EndIf
				;If (PlayerRoom <> e\room) And (e\EventState = 1) Then
				;	e\room\RoomDoors[0]\open = False
				;	e\room\RoomDoors[0]\locked = False
				;	PlaySound_Strict LoadTempSound("SFX\Door\DoorClose079.ogg")
				;	e\EventState = 2
				...
				If PlayerRoom = e\room Then
					...
					If EntityY(Collider)>1040.0*RoomScale Then
						...
						Else
							...
							UpdateSky(Sky,Camera)
							
							BlitzcordGameStatus=7
