;[Block]
Global Curr173.NPCs, Curr106.NPCs, Curr096.NPCs, Curr5131.NPCs, Curr049.NPCs, CurrZombie.NPCs, Curr008.NPCs
...
Const NPCtypeTrain% = 24

Const cannotTxt$ = "instances cannot be spawned with the console. Sorry!"
;[End Block]
...
Function CreateNPC.NPCs(NPCtype%, x#, y#, z#)
	...
	Select NPCtype
		...
		Case NPCtypeTrain
			;[Block]
			n\NVName = "Site-19 Mono-rail Train 0448"
			...
			temp# = (GetINIFloat("DATA\NPCs.ini", "Train", "scale") / MeshDepth(n\obj))			
			ScaleEntity n\obj, temp,temp,temp
			n\Speed = (GetINIFloat("DATA\NPCs.ini", "Train", "speed") / 10)
			;[End Block]
		...
		Case NPCtype173
			...
			;if you are infected by 3312, or you are a mentally insane (like this mod's creator), give 173 a fursona
			If furri Or RandomSeed = "FUNNIMAN_EXE" Or RandomSeed = "FUNNIMAN" Then
				furri = True
				Local temp2# = EntityScaleX(n\obj)
				FreeEntity(n\obj)
				n\obj = LoadAnimMesh_Strict("GFX\npcs\173bodyOwO.b3d")
				FreeEntity(n\obj3)
				n\obj3 = LoadAnimMesh_Strict("GFX\npcs\173headOwO.b3d")
				Animate n\obj, 1, 0.05
				;Animate n\obj3, 1, 0.5
				ScaleEntity n\obj, temp,temp,temp
				ScaleEntity n\obj3, temp,temp,temp
			EndIf
			
			;if the date is halloween, make 173 spoopy
			If (Left(CurrentDate(), 7) = "31 Oct ") Then ;enable halloween 173
				HalloweenTex = True
				Local texFestive = LoadTexture_Strict("GFX\npcs\173h.pt", 1)
				EntityTexture n\obj, texFestive, 0, 0
				EntityTexture n\obj3, texFestive, 0, 0
		...
		Case NPCtypeMTF
			;[Block]
			n\NVName = "Human - Mobile Task Force Epsilon-11"
			n\Collider = CreatePivot()
			EntityRadius n\Collider, 0.2
			;EntityRadius Collider, 0.15, 0.30
			EntityType n\Collider, HIT_PLAYER
		...
		Case NPCtypeD
			;[Block]
			n\NVName = "Human - Class-Disposable"
			n\Collider = CreatePivot()
		...
		Case NPCtype049
			...
			n\Speed2 = (GetINIFloat("DATA\NPCs.ini", "SCP-049", "spottedSpeed") / 100.0)
		...
		Case NPCtypeApache
			;[Block]
			n\NVName = "Mobile Task Force Epislon-11 Helicopter"
			n\GravityMult = 0.0
			n\MaxGravity = 0.0
			n\Collider = CreatePivot()
			EntityRadius n\Collider, 0.2
		...
		Case NPCtypeTentacle
			;[Block]
			n\NVName = "NVG ERROR: NULL REFERENCE EXPECTION"
		...
		Case NPCtype939
			...
			For n2.NPCs = Each NPCs
				...
				EndIf
			Next
			If amount939% = 0 Then i = 27
			If amount939% = 1 Then i = 41
			If amount939% = 2 Then i = 53
			If amount939% = 3 Then i = 84
			If amount939% = 4 Then i = 89
			If amount939% = 5 Then i = 92
			If amount939% = 6 Then i = 96
			n\NVName = "SCP-939-"+i
	...
	For n.NPCs = Each NPCs
		...
		Select n\NPCtype
			...
			Case NPCtypeTrain
				...
				;Because of how the train works this line isn't needed, although i may need it later to do testing shit
				;DebugLog "Pos for train is: "+EntityX(n\Collider)+", "+EntityY(n\Collider)+", "+EntityZ(n\Collider)+"."
				If n\State = 2 Then
					;For i=1 To 250
					;	PositionEntity(n\obj, EntityX(n\Collider)+5, EntityY(n\Collider), EntityZ(n\Collider))
					;Next
					AnimateNPC(n,1,250,n\Speed,False)
				EndIf
				;[End Block]
			...
			Case NPCtype173
				...
				If Curr173\Idle <> 3 Then
					...
					If n\Idle < 2 Then
						...
						PositionEntity(n\obj3, EntityX(n\Collider), EntityY(n\Collider) - 0.32, EntityZ(n\Collider))
						RotateEntity (n\obj3, 0, (EntityYaw(n\Collider)-180)+n\Angle, 0)
												...
												If w\door=Null And Rand(5)=1 Then
													...
													If x < 25.0 And x > 15.0 Then
														...
														If z < 25 And z > 15.0 Then
															...
															CreateConsoleMsg("moving 173's position to "+w\room\roomtemplate\name)
											...
											If KillTimer >= 0 And (Not GodMode) Then
												...
												Select PlayerRoom\RoomTemplate\Name
													Case "lockroom", "room2closets", "coffin"
														DeathMSG = "Subject D-9341. Cause of death: Fatal cervical fracture. Surveillance tapes confirm that the subject was killed by SCP-173."	
													Case "173"
			...
			Case NPCtype096
				...
				Select n\State
						...
						If dist<8.0 Then
							...
							;If (Not NoTarget) Then
								If angle<90 Or angle>270 Then
									If (NoTarget096=True) Then
										DebugLog "Protected From SCP-096"							
									Else
					...
					Case 4
						...
						;If (NoTarget=True) And n\Target = Null Then n\State = 5
						
						If KillTimer =>0 Then
							
							If n\State2 > 0 Then;stunned by telsa gate
								AnimateNPC(n,1333,1368,0.4,False)
								If n\Frame => 1367.9 Then n\State2 = 0	
								DebugLog n\Frame
							Else
						...
						Case 2 ;being active
							...
							If (dist < HideDistance*2) And (Not n\Idle) And PlayerInReachableRoom(True) Then
								...
								If PlayerSeeAble%=True Or n\State2>0 Then ;Player is visible for 049's sight - attacking
									n\CurrSpeed = CurveValue(n\Speed2, n\CurrSpeed, 20.0)
			...
			Case NPCtypeGuard ;------------------------------------------------------------------------------------------------------------------
				...
				Select n\State
					...
					Case 3 ;follows a path
						...
						EndIf
						;[End Block]
					Case 4 ;randomly look around. if see player, aim and shoot (goto state 1)
						;[Block]
					...
					Case 5 ;following a target
						...
						MoveEntity n\Collider, 0, 0, n\CurrSpeed * FPSfactor
						;[End Block]
					Case 6 ;nothing right now, will evencially be just wandering around
						
					Case 7 ;idle animation, not actually idle
						;[Block]
						AnimateNPC(n,77,201,0.2)
						;Animate2(n\obj, AnimTime(n\obj), 923, 1354, 0.2)
						;[End Block]
					Case 8 ;none, used for events that require custom behaviour
						
					Case 9 ;face head towards player
						;[Block]
						AnimateNPC(n,77,201,0.2)
					...
					Case 10 ;walk foward
						...
						MoveEntity n\Collider, 0, 0, n\CurrSpeed * FPSfactor
						;[End Block]
					Case 11 ;combat walk randomly, shoot if found player
						;[Block]
						If n\Frame < 39 Or (n\Frame > 76 And n\Frame < 245) Or (n\Frame > 248 And n\Frame < 302) Or n\Frame > 344
							AnimateNPC(n,345,357,0.2,False)
							If n\Frame >= 356 Then SetNPCFrame(n,302)
						...
						If KillTimer => 0 Then
							...
							If dist < 11.0
								If EntityVisible(n\Collider,Collider) And NoTarget = False
									SearchPlayer = True
								EndIf
							EndIf
								...
								If n\Reload = 0
									...
									If PickedEntity() = Collider Or n\State3=1 Then
										...
										If PlayerRoom\RoomTemplate\Name = "start"
											DeathMSG = "Subject D-9341 found dead in [REDACTED] of Site-[REDACTED]. Autopsy report shows multiple bullet wounds "
											DeathMSG = DeathMSG + "penetrating the Subject's [REDACTED]. Security camera footage confirmed that the Subject was shot "
											DeathMSG = DeathMSG + "by a Facility Security Guard."
										ElseIf PlayerRoom\RoomTemplate\Name = "173"
											DeathMSG = "Subject D-9341 was killed in the Class-D holding area at Site-[REDACTED]. The subject was shot dead prior "
											DeathMSG = DeathMSG + "to the containment breach by a rogue Facility Security Guard. The guard in question was demoted to D-Class."
										Else
											If PlayerZone = 1
												DeathMSG = "Subject D-9341 found dead in [REDACTED] of Site-[REDACTED]. Autopsy report shows multiple bullet wounds "
												DeathMSG = DeathMSG + "penetrating the Subject's [REDACTED]."
											ElseIf PlayerZone = 2
												DeathMSG = "Subject D-9341 found dead in [REDACTED] of Site-[REDACTED] Heavy Containment Zone. Autopsy report shows multiple bullet wounds "
												DeathMSG = DeathMSG + "penetrating the Subject's [REDACTED]. Found on the subject's body were [REDACTED] and [REDACTED] level keycards."
											ElseIf PlayerZone = 3
												DeathMSG = "Subject D-9341 found dead in [REDACTED] of Site-[REDACTED]. The subject had managed to make it "
												DeathMSG = DeathMSG + "a fair amount into the Site's Entrance Zone without sustaining damage. Autopsy report showed multiple bullet "
												DeathMSG = DeathMSG + "wounds penetrating the Subject's [REDACTED]."
											EndIf
										EndIf
					...
					Case 12 ;combat idle
						...
						;[End Block]
					Case 13 ;combat idle with gun low
						;[Block]
						AnimateNPC(n,202,244,0.35,True)
						;[End Block]
					Case 14 ;combat walk (if path exists)
						;[Block]
						If n\PathStatus = 2 Then
							n\State = 13
							n\CurrSpeed = 0
						...
						EndIf
						;[End Block]
					Case 18 ;put away paper
						;[Block]
						AnimateNPC(n,608.0,622.0,0.4)
						
						If n\Frame >= 621
							n\State = 9
						End If
						;[End Block]
					Case 19 ;put away paper then phone
						;[Block]
						AnimateNPC(n,608.0,622.0,0.4)
						...
						End If
						;[End Block]
					Case 20 ;turn on phone
						;[Block]
						AnimateNPC(n,665.0,710.0,0.4)
					...
					Case 21 ;phone
						;[Block]
						AnimateNPC(n,711.0,811.0,0.4)
						;[End Block]
					Case 22 ;turn off phone
						;[Block]
						AnimateNPC(n,812.0,857.0,0.4)
						
						If n\Frame >= 856
							n\State = 7
						End If
						;[End Block]
					Case 23 ;turn off phone, then look at player
						;[Block]
						AnimateNPC(n,812.0,857.0,0.4)
						...
						End If
						;[End Block]
					Case 24 ;turn off phone, then default
						;[Block]
						AnimateNPC(n,812.0,857.0,0.4)
							...
							If n\LastSeen > 0 And (Not NoTarget) Then
								...
								If (n\Frame=>18.0 And n\Frame<68.0) Then
									...
									If Injuries>4.0 Then 
										DeathMSG=Chr(34)+"All six (6) escaped SCP-939 specimens have been captured and recontained successfully. "
	...
	If n\NPCtype<>NPCtypeMTF Then
		...
		Select n\NPCtype
			...
			Case NPCtypeD
                realType = "a d-class"
			Case NPCtype372
                realType = "372 for some reason"
			Case NPCtypeApache
                realType = "a fucking apache helicopter, what the fuck is wrong with you"
			Case NPCtype096
                realType = "096"
			Case NPCtype049
                realType = "049"
			...
			Case NPCtype178
                realType = "178"
			Case NPCtypePdPlane
                realType = "the fucking pocket dimention plane, like bro wtf"
			Case NPCtype966
                realType = "966"
			Case NPCtype1048a
                realType = "1048-A"
			Case NPCtype1499
				realType = "1499-1"
			Case NPCtypeTrain
				realType = "a fucking train that only appears once in the entire game"
		End Select
								...
								For r = Each Rooms
									If r\RoomTemplate\Name$ = "start"
										...
										Else
											...
											PlayAnnouncement("SFX\Character\MTF\Announc173Contain"+Rand(1,3)+".ogg")
...
Function Console_SpawnNPC(c_input$, c_state$ = "")
	...
	Select c_input$ 
		Case "008", "008zombie"
			...
			Curr008 = n
			If c_state = "" Then c_state = "1"
			n\State = Int(c_state)
			consoleMSG = "New SCP-008 infected human spawned with state: "+c_state+"."
		...
		Case "049", "scp049", "scp-049"
			...
			If c_state = "" Then c_state = "1"
			n\State = Int(c_state)
			consoleMSG = "New SCP-049 instance spawned with state: "+c_state+"."
		...
		Case "049-2", "0492", "scp-049-2", "scp049-2", "049zombie"
			...
			If c_state = "" Then c_state = "1"
			n\State = Int(c_state)
			consoleMSG = "New SCP-049-2 instance spawned with state: "+c_state+"."
			
		Case "066", "scp066", "scp-066"
			n.NPCs = CreateNPC(NPCtype066, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "New SCP-066 instance spawned."
			
		Case "npc178", "scp-178-2", "scp178-2", "178"
			CreateConsoleMsg("SCP-178-2 "+cannotTxt, 255, 0, 0)
			
		Case "096", "scp096", "scp-096"
			n.NPCs = CreateNPC(NPCtype096, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			If c_state = "" Then c_state = "5"
			n\State = Int(c_state)
			If (Curr096 = Null) Then Curr096 = n
			consoleMSG = "New SCP-096 instance spawned with state: "+c_state+"."
		...
		Case "106", "scp106", "scp-106", "larry"
			...
			Curr106\State = -1
			If temporary=1 Then
				consoleMSG = "New SCP-106 instance spawned."
				temporary=0
			Else
		...
		Default 
			If c_input = ""
				CreateConsoleMsg("Please specify an NPC Type", 255, 0, 0) : Return
			Else
				CreateConsoleMsg(c_input+" is not a valid NPC Type", 255, 0, 0) : Return
			EndIf
	End Select
